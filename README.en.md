...
Script UI is localized: **Russian** and **English** (`ru` / `en`), with auto-detect from the system language.
...
- Languages: `ru`, `en`, `auto` (Russian on Russian systems, otherwise English)
- Default setting: `config/project.ini`
- CLI override: `--language ru|en` (CMD) or `-Language ru|en` (PowerShell)
...
| `--language ru\|en` | UI language (default: `auto`) |
...
| `-Language ru\|en` | UI language |
| `--language ru\|en` | Same (for the CMD launcher) |
...
| `--language ru\|en` | Message language |
...
LANGUAGE=auto          # fallback; primary language is config\project.ini
